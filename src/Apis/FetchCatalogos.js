...
const URL_COSTOS_SERVICIO = "http://suvalsa-home.ddns.net:8030/costosServicio/";
const METHOD_LISTAR = "listar";
const METHOD_ALTA = "crear";
const METHOD_BUSCAR = "ver/";
const METHOD_BORRAR = "borrar/";
const METHOD_BUSCARFAMRUT = "buscar/";
const METHOD_ACTUALIZARFAMRUT = "actualizar/";

//Post Costo Servicio
export async function AgregarCostoServicio(data) {
  let responseAlta = null;
  responseAlta = await FetchApiPost(
    URL_COSTOS_SERVICIO.concat(METHOD_ALTA),
    JSON.stringify(data)
  );
...
//Get Costo Servicio
export async function ObtenerCostoServicioRutaFamilia(ruta, familia) {
  try {
    const url = URL_COSTOS_SERVICIO.concat(METHOD_BUSCARFAMRUT).concat(ruta).concat("/").concat(familia);
    const dataCosto = await FetchApi(url);
    return dataCosto.data;
  } catch (error) {
    console.error("Error al obtener costos por servicio:", error);
    return []; // Retornar un arreglo vacío o manejar el error de manera apropiada
  }
}

//Update Costo Servicio
export async function ActualizarCostoServicioRutaFamilia(data) {
  try {
    const url = URL_COSTOS_SERVICIO.concat(METHOD_ACTUALIZARFAMRUT);
    const response = await FetchApiPost(url, JSON.stringify(data));
    return response;
  } catch (error) {
    console.error("Error al actualizar costos por servicio:", error);
    return null; // Retornar un arreglo vacío o manejar el error de manera apropiada
  }
}
